...
func compareVersion(version1 string, version2 string) int {
	...
	switch {
	case len(v1Split) > len(v2Split):
		gap := len(v1Split) - len(v2Split)
		for i := 0; i < gap; i++ {
			v2Split = append(v2Split, "0")
		}
	case len(v1Split) < len(v2Split):
		gap := len(v2Split) - len(v1Split)
		for i := 0; i < gap; i++ {
			v1Split = append(v1Split, "0")
		}
	}

	for i := 0; i < len(v1Split); i++ {
		a, _ := strconv.Atoi(v1Split[i])
		b, _ := strconv.Atoi(v2Split[i])
		if a > b {
			return 1
		} else if a < b {
			return -1
